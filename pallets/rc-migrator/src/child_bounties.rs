			...
			last_key = match last_key {
				...
				ChildBountiesStage::ParentChildBounties { parent_id } => {
					...
					match iter.next() {
						...
						None => {
							ChildBountiesStage::ChildBounties { ids: None }
						}
					}
				},
				ChildBountiesStage::ChildBounties { ids } => {
					let mut iter = if let Some((parent_id, child_id)) = ids {
						pallet_child_bounties::ChildBounties::<T>::iter_from(
							pallet_child_bounties::ChildBounties::<T>::hashed_key_for(&parent_id, &child_id)
						)
					} else {
				...
				stage => todo!("Unhandled stage: {:?}", &stage),
			};
		}
...
pub mod alias {
	...
	pub type ChildBountyOf<T> = ChildBounty<
